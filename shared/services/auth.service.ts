...
export const checkLogin = (): boolean => {
  return (
    IS_SERVER_LOCAL_STORAGE && localStorage.authInfo && localStorage.userInfo
  );
};
...
const getUserData = (): IUserData => {
  try {
    ...
    if (data) {
      const bytes = CryptoJS.AES.decrypt(data.toString(), KEY);
      const decryptedData: IUserData = JSON.parse(
...
const generateUserHash = (id: string) => {
  const hash = CryptoJS.HmacSHA256(
    id,
    'kYeOTa2xx3DxKjsLB9cJj43k7ZrdZk5XsijucP8N'
  );
